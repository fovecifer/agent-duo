...
5. 启动 tmux 会话:
   - claude 窗口:若已授权/已存在块 → 用 claude --append-system-prompt "$(指令正文)" 启动;
                  若拒绝 → 直接 claude 启动。
   - codex 窗口:始终 codex 启动(它从 AGENTS.md 自行读取,已授权则块已就位)。
```
...
- **无 TTY**(stdin 非交互,如管道/CI):无法询问。默认**不注入**,打印警告 + 手动说明,
  仍启动两个窗口。提供逃生通道 `AGENT_DUO_AUTO_INJECT=1`(或 `-y` 标志):跳过询问、
  直接注入,供自动化使用。
