...
- **`ls` 与 `show` 都支持 `--round N` 与 `--json`**；二者 `--json` 共用 §下方同一 wrapper schema，区别仅在 `show` 的 `result` 字段会带完整结果对象、`ls` 同样可带（schema 不分叉，消费方稳定）。
...
- `cmd` / `satisfies` 取自冻结契约的闸门声明；**`satisfies` 为数组**（与 contract/result 一致，
  `lib/loop.sh:973` 已 `map(tostring)` 规整，空时为 `[]`）；`status` 按上表映射（来自 `results[]` 匹配项）；
...
- **judge 不接受 `--needs* / --type / --target-ref` 之外的请求类 flag**：`--needs`/`--needs-kind`/
  `--needs-detail`/`--needs-option` 属于 `peer report --type request`（开 Human Decision Gate）的语义，
  与「记录一条 verdict」无关，judge 一律不暴露；`--type` 由 judge 固定注入 `result`，不接受外部覆盖。
  `<target-ref>` 即位置参数（等价旧 `--target-ref`）。
