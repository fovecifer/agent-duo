...
6. **用户可见输出标签同步改词（决策项）**
   ...
   - **边界**：仅改**显示字符串**；**on-disk 不动**——event type 仍为 `review_required`、JSON 字段名
     （`validation`/`status`/`verdict` 等）、`.agent-duo/` 记录结构均不变（守 §5「换门面不换地基」）。
   - 受影响的输出断言（如集成测试里 `validation=pass accept=reviewer:vetoed(...)` 一行）**与显示改词锁步更新**。
...
| 范围 | 动作 |
|---|---|
| `bin/peer` 调度器 | 重写参数路由为「名词-动词」两级分发；transport 与 steering 保持顶层单级；`agent`/`approval`/`verify`/`judge`/`gate`/`budget`/`task` 走两级；**`loopd` 顶层分支必须原样保留**（运行时入口） |
| 名词内小 flag 保留 | `gate ls [--all]`、`approval ls [--all]`、`approval deny [--reason …]`、`task show [<id>]`、`wait <id> --round N [--timeout] [--interval]`、`tell/esc [--force]`——迁到新动词时**逐个保留原 flag/变体**，不得丢失 |
| 用户可见标签改词 | `loop_print`/`checkpoint`/loopd dashboard 的 `VALIDATION`/`ACCEPTANCE`/`validation=`/`accept=` → `VERIFY`/`JUDGE`/`verify=`/`judge=`（仅显示串，on-disk event/字段不动，见 §4.6），相关输出断言锁步改 |
