...
| 顶层用法横幅 | `bin/peer` 顶层 `help\|*)` 默认分支打印的总用法/命令清单按三层 + 名词布局重写（`peer` 裸跑 / `peer --help` 都走这里），与新命令面一致 |
| 启动引导文案 | `start.sh` 里的 `peer broker-check` 引导、`bin/peer` 内 `peer add` 之后及 broker 硬门拒发时打印的提示串改为新命令名。grep `broker-check`/`broker-status`/`peer approvals`/`peer approve`/`peer deny`/`peer ls`/`peer loop <id>` 逐处替换，**范围限定：代码（`bin/`、`scripts/`、`*.sh`）、测试、当前 agent 指令（`docs/AGENT-INSTRUCTIONS.md` 注入块）、本次新建/改动文档；不含 `docs/superpowers/specs/` 历史设计**（与 §4 glossary 项「历史 specs 不回改」一致），避免引导用户敲已删除命令 |
...
4. **无别名负向测试（显式断言旧命令已删）**：`peer ls`、`peer loop <id>`、`peer task <id>`、
   `peer approvals`、`peer approve`、`peer deny`、`peer broker-check`、`peer broker-status`、
   `peer report --verdict …` 逐条**非 0 退出**，且 stderr **提示对应新命令**（如 `peer ls`→提示 `peer agent ls`、
   `peer report --verdict`→提示 `peer judge`）。这组负向用例是「旧命令面真删干净」的可执行证据。
...
- **排序**：`target_round` 倒序为主键；同一 `target_round` 内按记录时间倒序；仍并列时以 `role` 升序兜底，
  保证**确定性**（测试可稳定断言顺序）。
- **存在性按 state 判定，不依赖 pane 注册**（与 `task_print`/`loop_print` 等只读命令同口径——按
  `.agent-duo/state/<id>/` 读，移除的 agent 历史 verdict 仍可查）：
  - `state/<id>/` 目录**不存在** → **非 0 退出** + 提示「没有该 agent 的 state」（仿 `没有 loop.json …`）。
  - `state/<id>/` 存在但 `reviews/` 无任何 verdict 记录 → **退出 0**，文本打印空集提示、`--json` 输出 `[]`。
