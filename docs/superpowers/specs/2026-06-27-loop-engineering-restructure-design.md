...
| `peer report --type result --verdict approve --target-ref worker@N [--finding …]` | `peer judge <target-ref> --verdict approve [--finding …]` / `peer judge ls <id>` | 判定从 report 拆出，独立对抗式评判成独立名词（**产出物与行为不变**，见 §6.2） |
...
- `peer verify ls <id>` — 列出该 worker **冻结契约里声明的 verify 闸门 id**，每条附**最近一轮验证结果**
  （runtime 已写的 `validation-rN.json` 取最新 N 的 pass/fail/satisfies 摘要）。无契约时非 0 退出并提示先 `peer loop init`。
- `peer verify show <id> [--round N] [--json]` — 展开单个 worker：默认显示**契约声明的闸门**
  + **最新一轮**每个闸门的结果记录；`--round N` 指定轮次；`--json` 输出原始记录供脚本消费。
- 这两个命令**只读**，不触发验证执行（验证由 loop runtime 在轮次推进时跑，本次不引入手动 `verify run`）。

### 6.2 `peer judge`（产出物与现状一致，仅换命令名）

`peer judge <target-ref> --verdict … [--finding …]` 的落盘行为**与今天的
`report --type result --verdict` 完全一致**（呼应「换门面不换地基」），即同时产生：

1. 评判方自身的 report 记录（`state/<reviewer-role>/rN.json`，含 `verdict` / `target_ref` / `findings`）;
2. 路由到目标 worker 的 verdict 记录（`state/<worker>/reviews/<reviewer-role>-r<targetN>.json`，含
   `verdict` / `by` / `role` / `target_round`）;
3. 既有的 sentinel + runtime event（命中目标 loop `veto_on` 时保持 worker active 并发 `review_required`）。

`--target-ref worker@N` 的解析、耦合校验（缺 `--target-ref` 即 fail-closed、不写任何 report）复用
`bin/peer` 既有逻辑。`peer judge ls <id>` 列出路由到该 worker 的全部 verdict 记录（按 target_round / 时间倒序）。
因此 `test/peer.test.sh` 中针对 verdict 落盘的断言只需把被测命令从 `report --type result --verdict`
改成 `judge`，断言的文件路径与字段不变。
