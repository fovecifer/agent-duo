...
```
peer judge <target-ref> --round R --verdict V [--status done] [--finding sev:note]… \
           [--delta …] [--evidence-cmd/--evidence-result/--evidence-ref …]
```

- **必填**：位置参数 `<target-ref>`（= `--target-ref worker@N`）、`--round R`（评判方**自己**的轮次，
  对应集成测试里 reviewer 的 `--round 1` / `--round 2`）、`--verdict V`。
