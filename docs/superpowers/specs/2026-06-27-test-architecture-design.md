...
test/
  lib/
    assert.sh                       # ← 现 test/assert.sh 平移
    harness.sh                      # ← 新建：tmux stub / make_tmp / setup·teardown
                                    #   / run_peer / run_peer_as / run_loopd_once / registry fixture
  unit/
    loop.test.sh                    # ← 现 loop.test.sh 平移
  ...
  integration/
    supervisor-loop.test.sh         # ← 现 integration.test.sh 的分项断言
...
**输出形态（示意）**
```
── unit ──────────────  loop ✓  broker ✓  registry ✓  inject ✓
...
=====================================================
unit 4/4  cli 12/12  integration 3/3  e2e 0/3(3 skipped)
ALL TESTS PASSED
```
