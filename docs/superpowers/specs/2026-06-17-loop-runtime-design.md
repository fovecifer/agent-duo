...
Claude Code 与 Codex 都有 lifecycle hook，daemon 不靠抓屏猜 turn 状态，而是给 supervisor session 挂 command hook 拿权威信号。注意：事件名相近不代表输出语义完全同构，Approval Broker 必须走 provider adapter（Codex 侧尤其不能把 `ask` 当升级人工确认）。

| hook | 触发 | daemon 用途 |
|---|---|---|
| `UserPromptSubmit` | 一个 turn 开始 | 标记 **busy，别注入** |
| `Stop` | 一个 turn 结束、即将 idle | 标记 **idle**，且可借此投递事件（见下） |
| `PreToolUse` / `PermissionRequest` | 工具执行前 / 权限弹窗 | （Approval Broker 用，见 [Approval Broker 设计](./2026-06-17-approval-broker-design.md)） |
...
5. **hook 安装**：supervisor session 需挂 `UserPromptSubmit`/`Stop`（busy/idle + Stop 便车），worker session 挂 `PreToolUse`/`PermissionRequest`（Approval Broker）。agent-duo 自带这些 hook 脚本，`start.sh` / `peer add` 创建 session 时注入对应 settings；Codex 侧需要处理非托管 hook 信任，首次未信任时不得假设 broker 已生效。
...
- ~~**Approval Broker 的 hook 化**~~ → worker session 的 `PreToolUse` hook，三结局 auto-allow/escalate/hard-deny，详见 [Approval Broker 设计](./2026-06-17-approval-broker-design.md)。

设计阶段已全部闭环；后续转入实现（见 §11 增量落地）。
