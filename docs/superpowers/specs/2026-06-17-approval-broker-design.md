...
- **Codex 侧**：对应 `PreToolUse` / `PermissionRequest`，设计相同。注意 Codex 的"非托管 hook 需审核信任"要求——`peer add` / `agent-duo-start --with` 装 hook 时按 Codex 的 hook trust 机制处理。
...
- **装 hook**：`peer add` / `agent-duo-start --with` 创建 worker pane 时，把 broker 的 `PreToolUse` hook 写入该 session 的 settings；Claude 通过 `--settings` 加载，Codex 通过临时 `-c hooks.PreToolUse=...` 配置加载并走自身 hook 信任机制。
